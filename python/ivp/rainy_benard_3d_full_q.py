...
This script solves IVPs, starting from an analytic base state, using full q1, b1 that include the background q0, b0.
...
logger.info('solving for full (not fluctuating) variables q, b')
b = b1
q = q1
...
if not args['--no-output']:
    ...
    for z_frac in [0.25, 0.5, 0.75]:
        ...
        snapshots.add_task(ey@u(z=Lz*z_frac), name=f'uy {z_frac} z')
    ...
    slices.add_task((ex@u)(z=0.5, y=0.5), name='ux')
    slices.add_task((ey@u)(z=0.5, y=0.5), name='uy')
    slices.add_task((np.sqrt((u-h_avg(u))@(u-h_avg(u))))(z=0.5, y=0.5), name='u_rms')
    slices.add_task((ey@ω)(z=0.5, y=0.5), name='omega_y')
