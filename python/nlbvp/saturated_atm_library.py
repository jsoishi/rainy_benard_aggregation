...
Options:
    ...
    --end_tau=<et>       Ending value for tau sweep [default: 1e-7]
    --num_tau=<nt>       Number of taus to sample in tau sweep [default: 9]
...
data_dir = 'saturated_atm_alpha{:}_beta{:}_gamma{:}'.format(args['--alpha'], args['--beta'], args['--gamma'])
...
for i, tau_i in enumerate(taus):
    case_dir = 'tau_{:.2g}_nz{:d}'.format(tau_i, nz)
    if args['--erf']:
        case_dir += '_erf'
    if args['--Legendre']:
        case_dir += '_Legendre'
    ...
    if need_guess:
        logger.info('solving tau={:}, starting from analytic'.format(tau_i))
        b.change_scales(dealias)
        q.change_scales(dealias)
        b['g'] = analytic['b']['g']
        q['g'] = analytic['q']['g']
    ...
    while pert_norm > tol and solver.iteration <= stop_iteration:
        ...
        logger.info("tau = {:.1g}, L2 err = {:.1g}".format(tau['g'][0,0,0], pert_norm))
