...
for kx in kxs:
    ...
    plt.scatter(kx_plot[-1], waves[-1].imag, color='k')

kz = 6.28
N = np.sqrt(0.1)
def gw_disp(kx, kz, N):
    return N*kx/np.sqrt((kx**2 + kz**2))
plt.loglog(kxs, gw_disp(kxs,kz, N), label=r'$N_b \frac{kx}{\sqrt{kx^2 + (2\pi/Lz)^2}}$')
plt.legend()
#plt.ylim(-0.4,0.4)
plt.ylim(1e-3,0.6)
plt.xlabel(r"$k_x$")
...
fig_filename=f"Ra_{Rayleigh:.2e}_nz_{nz}_bc_{bc_type}_dense_{dense}_freq_vs_kx"
