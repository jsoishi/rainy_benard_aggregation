...
def evp_amp_reject(evp, indices, tol=1e-8):
    amp = []
    for i in indices:
        evp.solver.set_state(i,evp.solver.subsystems[0])
...
def plot_eigenmode(evp, index, mode_label=None):
    evp.solver.set_state(index,evp.solver.subsystems[0])
    ...
    for v in ['b','q','p']:
        if v == 'b':
            ...
            phase_correction = evp.fields[v]['g'][...,i_max].squeeze()
        evp.fields[v].change_scales(1)
        name = evp.fields[v].name
        data = evp.fields[v]['g'].squeeze()/phase_correction
    ...
    axes['ux'].plot(u[0,...,:].squeeze().real, z)
    axes['ux'].plot(u[0,...,:].squeeze().imag, z,':')
    axes['ux'].set_xlabel(r"$u_x$")
    axes['ux'].set_ylabel(r"$z$")
    axes['uz'].plot(u[-1,...,:].squeeze().real, z)
    axes['uz'].plot(u[-1,...,:].squeeze().imag, z, ':')
...
if __name__ == "__main__":
    ...
    if args['--tau']:
        tau_in = float(args['--tau'])
    # build solvers
    lo_res = RainyBenardEVP(nz, Rayleigh, tau, kx, γ, α, β, lower_q0, k, relaxation_method=relaxation_method, Legendre=Legendre, erf=erf, bc_type=bc_type, nondim=nondim, dealias=dealias,Lz=1, twoD=twoD)
    lo_res.plot_background()
    if args['--rejection_method'] == 'resolution':
        hi_res = RainyBenardEVP(int(2*nz), Rayleigh, tau, kx, γ, α, β, lower_q0, k, relaxation_method=relaxation_method, Legendre=Legendre, erf=erf, bc_type=bc_type, nondim=nondim, dealias=dealias,Lz=1, twoD=twoD)
        hi_res.plot_background()
    elif args['--rejection_method'] == 'bases':
        hi_res = RainyBenardEVP(nz, Rayleigh, tau, kx, γ, α, β, lower_q0, k, relaxation_method=relaxation_method, Legendre=not(Legendre), erf=erf, bc_type=bc_type, nondim=nondim, dealias=dealias,Lz=1, twoD=twoD)
